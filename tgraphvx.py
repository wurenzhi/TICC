...
def getValue(arr, index, length):
    return np.array(arr[index:(index + length)])
...
def ADMM_x(entry):
    ...
    if (__builtin__.len(entry[1].args) > 1):
        ...
        solution = np.array(x_update).T.reshape(-1)
...
def ADMM_z(entry, index_penalty=1):
    ...
    z_ij = np.zeros(probSize * (probSize + 1) / 2)
        ...
        if (i == 0):
            ...
            for j in range(sizeBlocks):
                for k in range(j, sizeBlocks):
                    ...
                    ansPos = max((rho * np.sum(points) - lamSum) / (rho * elems), 0)

                    # If answer is negative
                    ansNeg = min((rho * np.sum(points) + lamSum) / (rho * elems), 0)

                    if (rho * np.sum(points) > lamSum):
                        for locs in locList:
                            index = ij2symmetric(locs[0], locs[1], probSize)
                            z_ij[index] = ansPos
                    elif (rho * np.sum(points) < -1 * lamSum):
                        for locs in locList:
                            index = ij2symmetric(locs[0], locs[1], probSize)
                            z_ij[index] = ansNeg
                    else:
